...
function BlogDetail({ userData, setUserData }) {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        const res = await fetch(`https://codru-backend.vercel.app/blogs/${blogId}`);
        if (!res.ok) {
        ...
        const userRes = await fetch(`https://codru-backend.vercel.app/users/${currentUser}`);
  ...
  const handleLike = async () => {
    try {
      const res = await fetch("https://codru-backend.vercel.app/blog/like", {
        method: "POST",
  ...
  const handleDislike = async () => {
    try {
      const res = await fetch("https://codru-backend.vercel.app/blog/dislike", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blogId, username: currentUser }),
      });
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(
        `https://codru-backend.vercel.app/blog/${isSaved ? "unsaveblog" : "saveblog"}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
