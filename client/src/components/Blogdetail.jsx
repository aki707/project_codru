...
function BlogDetail({ userData, setUserData }) {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        const res = await fetch(`https://codru-server.vercel.app/blogs/${blogId}`);
        if (!res.ok) {
        ...
        const userRes = await fetch(`https://codru-server.vercel.app/users/${currentUser}`);
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(
        `https://codru-server.vercel.app/blog/${isSaved ? "unsaveblog" : "saveblog"}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
