...
function Commentpage({ onFocus, blogId, userData, setUserData }) {
  ...
  const fetchComments = async () => {
    try {
      const res = await fetch(`https://codru-server.vercel.app/comments/${blogId}`);
      if (!res.ok) {
  ...
  useEffect(() => {
    const updateUserPhoto = async () => {
      try {
        const res = await fetch("https://codru-server.vercel.app/updateUserPhoto", {
          method: "PUT",
  ...
  const deleteComment = async (commentId, blogId) => {
    try {
      const res = await fetch(`https://codru-server.vercel.app/comments/${commentId}`, {
        method: "DELETE",
        headers: {
  ...
  const dislikeComment = async (commentId) => {
    try {
      const res = await fetch("https://codru-server.vercel.app/comments/dislike", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blogId, commentId, username: currentUser }),
      });
