...
router.post("/add-subject", async (req, res) => {
    ...
    if (!section || !subjectID) {
      return res.status(400).json({ error: "Section and SubjectID are required." });
    }
    ...
    try {
      ...
      if (!["allsubjects", "skilldevelopment", "customcourses"].includes(section)) {
  ...
  router.post("/add-price", async (req, res) => {
    ...
    try {
      ...
      const subjectIndex = course[section].findIndex(subject => subject.subjectID === subjectID);
      if (subjectIndex === -1) {
        return res.status(404).json({ error: "Subject not found in the specified section." });
      }
  
      const newPrice = { duration: { value: durationValue, unit: durationUnit }, price };
